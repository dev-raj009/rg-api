// lib/smartFetch.js — v11 (native fetch, no node-fetch)
...
export async function smartFetch(endpoint, params = {}, isVideo = false, batchId = null) {
  ...
  if (tokens.length === 0) {
    return { status: 503, error: "No tokens in pool. Please login first via /api/login or Telegram bot." };
  }
  ...
  return { status: 403, error: "All tokens failed. Token may be expired." };
}
